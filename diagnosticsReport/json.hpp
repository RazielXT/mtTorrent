...
class JSON
{
    ...
    public:
        ...
        string dump( int depth = 1, string tab = "  ", string separator = "\n") const {
            string pad = "";
            ...
            switch( Type ) {
                ...
                case Class::Object: {
                    ...
                    for( auto &p : *Internal.Map ) {
                        if (!skip) s += "," + separator;
                        s += ( pad + "\"" + p.first + "\" : " + p.second.dump( depth + 1, tab, separator ) );
                        skip = false;
                    }
                    s += (separator + pad.erase( 0, 2 ) + "}" ) ;
                    return s;
                }
                case Class::Array: {
                    string s = "[";
                    ...
                    for( auto &p : *Internal.List ) {
                        if( !skip ) s += ", ";
                        s += p.dump( depth + 1, tab, separator );
                        skip = false;
                    }
                    s += "]";
                    return s;
