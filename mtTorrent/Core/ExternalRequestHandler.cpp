...
void ExternalRequestHandler::openMagnetLink(uint32_t requestId, std::string& link, std::function<void(bool,mtt::TorrentPtr)> onResult)
{
	...
	/*if (parsedTorrent.parseMagnetLink(link) == mtt::Status::Success)
	{
		...
		if (ptr->infoFile.info.files.empty())
		{
			...
			dlTask->dl.start(ptr, [this, dlTask, ptr, onResult](bool success)
			{
				if (success)
				{
					ptr->infoFile.info = mtt::TorrentFileParser::parseTorrentInfo(dlTask->dl.metadata.buffer.data(), dlTask->dl.metadata.buffer.size());
				}
